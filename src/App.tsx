...
// â”€â”€â”€ Supabase â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const STATUS_OPTIONS = ["Ideia","Roteiro","ProduÃ§Ã£o","EdiÃ§Ã£o","Agendado","Publicado","Cancelado"] as const;
const FLOW_STATUS = STATUS_OPTIONS.filter(s => s !== "Cancelado") as Status[];
const EXTRA_TIKTOK_FORMAT = "VÃ­deo extra TikTok";
...
const STATUS_COLORS: Record<Status, { bg: string; text: string; border: string; rowBg: string; calBg: string }> = {
  ...
  ProduÃ§Ã£o:   { bg: "#1f3155", text: "#93c5fd", border: "#3b82f6", rowBg: "rgba(59,130,246,.12)", calBg: "#1d4ed8dd" },
  EdiÃ§Ã£o:     { bg: "#27364a", text: "#67e8f9", border: "#0891b2", rowBg: "rgba(8,145,178,.12)", calBg: "#0e7490dd" },
...
const TEMPLATES: TemplateDef[] = [
  { id: "reels", label: "Reels Criando XP", rede: "Instagram", formato: "Reels", tema: "", icon: "ðŸŽ¬" },
  { id: "meme", label: "Post de meme", rede: "Instagram", formato: "Post", tema: "Meme", icon: "ðŸƒ" },
  { id: "caixinha", label: "Caixinha / Story", rede: "Instagram", formato: "Story", tema: "Caixinha", icon: "â”" },
  { id: "carrossel", label: "Carrossel D&D", rede: "Instagram", formato: "Carrossel", tema: "D&D", icon: "ðŸ“š" },
  { id: "tiktok", label: "VÃ­deo extra TikTok", rede: "TikTok", formato: EXTRA_TIKTOK_FORMAT, tema: EXTRA_TIKTOK_FORMAT, icon: "ðŸŽµ" },
  { id: "shorts", label: "YouTube Shorts", rede: "YouTube", formato: "Shorts", tema: "", icon: "â–¶ï¸" },
];

const LEAD_STATUS_OPTIONS = ["Novo lead","Em contato","Mesa alocada","Desistiu","Lista de espera","NÃ£o respondeu","NÃºmero errado","Menor de 18 anos ðŸ¼"];
const LEAD_STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  "NÃ£o respondeu":  { bg: "#2a2a2a", text: "#9ca3af", border: "#4b5563" },
  "NÃºmero errado":  { bg: "#3a2a1a", text: "#fbbf24", border: "#b45309" },
  "Menor de 18 anos ðŸ¼": { bg: "#3a1a2e", text: "#f9a8d4", border: "#be185d" },
};
...
function legacyStatus(status: unknown): Status {
  ...
  if (status === "Em produÃ§Ã£o") return "ProduÃ§Ã£o";
...
function prettyField(field: string): string {
  const map: Record<string,string> = { status:"Status", data:"Data", rede:"Rede", formato:"Formato", responsavel:"ResponsÃ¡vel", checklist:"Checklist", published_at:"PublicaÃ§Ã£o" };
  return map[field] || field;
}
function sourceText(lead: Lead): string {
  return [lead.origem, lead.utm_source, lead.utm_campaign, lead.influencer_codigo, lead.notas].filter(Boolean).join(" Â· ");
}
function makeChecklist(format: string): ChecklistItem[] {
  const labels = format === EXTRA_TIKTOK_FORMAT
    ...
    : format === "Reels" || format === "Shorts"
      ? ["Roteiro", "GravaÃ§Ã£o", "EdiÃ§Ã£o", "Legenda", "Capa", "Agendamento"]
      : format === "Carrossel"
        ? ["Copy", "Design", "Legenda", "Agendamento"]
        : format === "Story"
          ? ["Copy", "Arte/GravaÃ§Ã£o", "PublicaÃ§Ã£o"]
          : ["Copy/Roteiro", "ProduÃ§Ã£o", "Legenda", "Agendamento"];
...
// â”€â”€â”€ Banco â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function dbLoad(mes: number): Promise<Row[]> {
  // Usa o cliente autenticado do Supabase. Assim as policies de RLS enxergam
  // o usuÃ¡rio logado, em vez de receberem sempre a anon key no Authorization.
  ...
  // Compatibilidade de deploy: antes da migraÃ§Ã£o criar data_iso, ainda consegue
  // abrir o calendÃ¡rio usando o campo mes legado.
...
async function dbUpsert(row: Row): Promise<void> {
  ...
  if (!data || data.length === 0) {
    throw new Error("O Supabase nÃ£o salvou a postagem. Verifique as policies de INSERT/UPDATE/RLS da tabela postagens para usuÃ¡rios autenticados.");
  }
}
...
async function dbUpsertMany(rows: Row[]): Promise<void> {
  ...
  if (!data || data.length !== rows.length) {
    throw new Error("O Supabase nÃ£o salvou todas as postagens. Verifique as policies de INSERT/UPDATE/RLS da tabela postagens para usuÃ¡rios autenticados.");
  }
}
...
async function dbPatch(id: string, patch: Partial<Row>): Promise<void> {
  ...
  if (error) throw new Error(error.message);
  if (!data || data.length === 0) {
    throw new Error("O Supabase nÃ£o atualizou a postagem. Verifique a policy de UPDATE/RLS da tabela postagens para usuÃ¡rios autenticados.");
  }
}
...
async function dbDelete(id: string): Promise<void> {
  ...
  if (error) throw new Error(error.message);
  if (!data || data.length === 0) {
    throw new Error("O Supabase nÃ£o excluiu a postagem. Verifique a policy de DELETE/RLS da tabela postagens para usuÃ¡rios autenticados.");
  }
}
...
// â”€â”€â”€ UI base â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Central de Hoje â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TodayCenter({ rows, leads, loading, onOpenPost, onGoContent, onGoLeads }: {
  ...
  const production = rows.filter(r => ["Roteiro","ProduÃ§Ã£o","EdiÃ§Ã£o"].includes(r.status));
  ...
  const group = (title: string, items: Row[], empty: string) => <div className="cxp-card" style={{ padding:14 }}>
    ...
    {items.length === 0 ? <div className="cxp-muted" style={{ fontSize:12,padding:"12px 0" }}>{empty}</div> : items.slice(0,6).map(r => <button key={r.id} onClick={() => onOpenPost(r)} style={{ width:"100%",background:"transparent",border:"none",borderTop:"1px solid #211042",padding:"9px 0",cursor:"pointer",textAlign:"left",color:"inherit" }}>
      <div style={{ display:"flex",gap:8,alignItems:"center" }}><span>{REDE_ICONS[r.rede] || "ðŸ“„"}</span><div style={{ flex:1,minWidth:0 }}><div style={{ fontSize:12,color:"#e2d0ff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{r.tema || r.postagem}</div><div className="cxp-muted" style={{ fontSize:10 }}>{r.data || "Sem data"} Â· {r.formato || "sem formato"}</div></div><StatusBadge status={r.status}/></div>
    </button>)}
  </div>;

  if (loading) return <div style={{ padding:60,display:"flex",justifyContent:"center" }}><Spinner/></div>;
  return <div>
    <SectionTitle eyebrow="VisÃ£o operacional">Central de Hoje</SectionTitle>
    ...
    <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:12 }}>
      {group("ðŸ”¥ Precisa acontecer hoje", todayRows, "Nada pendente para hoje.")}
      {group("â° Atrasados", late, "Nenhum conteÃºdo atrasado. Um raro momento civilizatÃ³rio.")}
      {group("ðŸŒ¤ AmanhÃ£", tomorrowRows, "Nada marcado para amanhÃ£.")}
      {group("âš™ Em produÃ§Ã£o", production, "Nenhum conteÃºdo em produÃ§Ã£o agora.")}
      {group("ðŸ’¡ Banco de ideias", backlog, "O backlog estÃ¡ vazio.")}
      <div className="cxp-card" style={{ padding:14 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}><div className="cxp-title" style={{ fontSize:12,color:"#c084fc" }}>ðŸ‘¥ Leads novos</div><span className="cxp-chip">{newLeads.length}</span></div>
        {newLeads.slice(0,6).map(l => <div key={l.id} style={{ borderTop:"1px solid #211042",padding:"9px 0" }}><div style={{ fontSize:12 }}>{l.nome || "Sem nome"}</div><div className="cxp-muted" style={{ fontSize:10 }}>{l.whatsapp_discord || "sem contato"} Â· {l.origem || l.utm_source || "origem nÃ£o informada"}</div></div>)}
        {newLeads.length === 0 && <div className="cxp-muted" style={{ fontSize:12,padding:"12px 0" }}>Nenhum lead novo.</div>}
      </div>
    </div>
    <div style={{ display:"flex",gap:8,marginTop:14,flexWrap:"wrap" }}><button className="cxp-btn primary" onClick={onGoContent}>Abrir conteÃºdo</button><button className="cxp-btn" onClick={onGoLeads}>Abrir CRM</button></div>
  </div>;
}

// â”€â”€â”€ CalendÃ¡rio â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function CalendarBoard({ rows, mes, scope, weekIndex, onWeekIndex, onMovePost, onOpen, onCreateForDay }: {
  ...
  return <div>
    {scope === "semana" && <div style={{ display:"flex",justifyContent:"flex-end",gap:6,marginBottom:8 }}><button className="cxp-btn" disabled={currentWeek===0} onClick={()=>onWeekIndex(Math.max(0,currentWeek-1))}>â€¹ Semana</button><button className="cxp-btn" disabled={currentWeek===maxWeek} onClick={()=>onWeekIndex(Math.min(maxWeek,currentWeek+1))}>Semana â€º</button></div>}
    ...
    <div className="cxp-calendar">{visible.map((day,idx) => {
      ...
      return <div key={day} className={`cxp-day ${day===todayDay?"today":""} ${dragOverDay===day?"drop":""}`}
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}><span style={{ font:"700 10px 'Cinzel',serif",color:day===todayDay?"#c084fc":"#6f5a95" }}>{day}</span><button onClick={()=>onCreateForDay(iso)} title="Criar conteÃºdo neste dia" style={{ background:"none",border:"none",color:"#6f5a95",cursor:"pointer",fontSize:14 }}>ï¼‹</button></div>
        {items.slice(0,5).map(r=>{const c=STATUS_COLORS[r.status];return <div key={r.id} draggable onDragStart={e=>e.dataTransfer.setData("rowId",r.id)} onClick={()=>onOpen(r)} className="cxp-daypost" title={`${r.tema||r.postagem} Â· ${r.formato}`} style={{ background:c.calBg,border:`1px solid ${c.border}`,color:c.text }}>{REDE_ICONS[r.rede]||"ðŸ“„"} {r.formato===EXTRA_TIKTOK_FORMAT?"Extra TikTok":r.tema||r.postagem}</div>})}
...
function Backlog({ rows, onOpen, onAdd }: { rows:Row[]; onOpen:(r:Row)=>void; onAdd:()=>void }) {
  const backlog=rows.filter(r=>!dateInputValue(r)&&!isDone(r.status));
  return <div className="cxp-card" style={{ padding:10,minWidth:230 }}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc"}}>ðŸ’¡ Banco de Ideias</div><button className="cxp-btn" onClick={onAdd}>ï¼‹</button></div><div className="cxp-muted" style={{fontSize:10,marginBottom:8}}>Arraste uma ideia para um dia do calendÃ¡rio.</div>{backlog.length===0&&<div className="cxp-muted" style={{fontSize:11,padding:12,textAlign:"center"}}>Sem ideias soltas.</div>}{backlog.map(r=><div key={r.id} draggable onDragStart={e=>e.dataTransfer.setData("rowId",r.id)} onClick={()=>onOpen(r)} className="cxp-kanban-card" style={{cursor:"grab"}}><div style={{fontSize:11,color:"#e2d0ff"}}>{r.tema||r.postagem}</div><div className="cxp-muted" style={{fontSize:9,marginTop:4}}>{REDE_ICONS[r.rede]||"ðŸ“„"} {r.formato||"sem formato"}</div></div>)}</div>;
}

// â”€â”€â”€ Kanban â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function KanbanView({ rows, onOpen, onStatus }: { rows:Row[]; onOpen:(r:Row)=>void; onStatus:(id:string,status:Status)=>void }) {
  const [over,setOver]=useState<Status|null>(null);
  return <div className="cxp-kanban">{FLOW_STATUS.map(status=>{const c=STATUS_COLORS[status];const items=rows.filter(r=>r.status===status);return <div key={status} className="cxp-kanban-col" onDragOver={e=>{e.preventDefault();setOver(status)}} onDragLeave={()=>setOver(null)} onDrop={e=>{e.preventDefault();const id=e.dataTransfer.getData("rowId");if(id)onStatus(id,status);setOver(null)}} style={{outline:over===status?`2px dashed ${c.border}`:"none"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><StatusBadge status={status}/><span className="cxp-muted" style={{fontSize:10}}>{items.length}</span></div>{items.map(r=><div key={r.id} draggable onDragStart={e=>e.dataTransfer.setData("rowId",r.id)} onClick={()=>onOpen(r)} className="cxp-kanban-card" style={{borderLeft:`3px solid ${c.border}`}}><div style={{fontSize:12,fontWeight:700,color:"#e2d0ff",marginBottom:4}}>{r.tema||r.postagem}</div><div className="cxp-muted" style={{fontSize:10}}>{r.data||"Backlog"} Â· {REDE_ICONS[r.rede]||"ðŸ“„"} {r.formato||"â€”"}</div>{r.checklist.length>0&&<div style={{marginTop:7}}><div className="cxp-progress"><div style={{width:`${Math.round(r.checklist.filter(i=>i.done).length/r.checklist.length*100)}%`}}/></div></div>}</div>)}</div>})}</div>;
}

// â”€â”€â”€ Tabela / aÃ§Ãµes em massa â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function PostDrawer({ row, onClose, onUpdate, onToggleChecklist, onAdapt, onRemove }: { row:Row; onClose:()=>void; onUpdate:(id:string,key:keyof Row,val:any)=>void; onToggleChecklist:(id:string,itemId:string)=>void; onAdapt:(r:Row)=>void; onRemove:(id:string)=>void }) {
  ...
  return <><div className="cxp-overlay" onClick={onClose}/><aside className="cxp-drawer">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:14}}><div><div className="cxp-label">Painel da postagem</div><div className="cxp-title" style={{fontSize:18,color:"#e9d5ff"}}>{row.tema||row.postagem}</div></div><button className="cxp-btn" onClick={onClose}>âœ•</button></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:12}}><div><div className="cxp-label">Data</div><input className="cxp-input" type="date" value={dateInputValue(row)} onChange={e=>onUpdate(row.id,"data",isoToBR(e.target.value))}/></div><div><div className="cxp-label">Status</div><select className="cxp-input" value={row.status} onChange={e=>onUpdate(row.id,"status",e.target.value)}>{STATUS_OPTIONS.map(s=><option key={s}>{s}</option>)}</select></div><div><div className="cxp-label">Rede</div><select className="cxp-input" value={row.rede} onChange={e=>onUpdate(row.id,"rede",e.target.value)}><option value="">â€”</option>{REDE_OPTIONS.map(s=><option key={s}>{s}</option>)}</select></div><div><div className="cxp-label">Formato</div><select className="cxp-input" value={row.formato} onChange={e=>onUpdate(row.id,"formato",e.target.value)}><option value="">â€”</option>{FORMATO_OPTIONS.map(s=><option key={s}>{s}</option>)}</select></div></div>
    <div style={{display:"grid",gap:10}}>{field("Tema","tema",false,"Tema do conteÃºdo")}{field("Hook","hook",true,"A primeira frase / ideia que prende atenÃ§Ã£o")}{field("Roteiro / legenda","roteiro",true,"Roteiro, legenda ou copy principal")}{field("CTA","cta",true,"O que a pessoa deve fazer depois")}{field("Briefing","briefing",true,"Contexto, objetivo e direcionamento")}{field("ReferÃªncias","referencias",true,"Links, ideias, referÃªncias visuais")}{field("ResponsÃ¡vel","responsavel",false,"Nome")}{field("Arquivos","link_arquivo",true,"Links do Drive separados por vÃ­rgula")}{field("ObservaÃ§Ãµes","observacoes",true,"AnotaÃ§Ãµes internas")}</div>
    <div className="cxp-card" style={{padding:12,marginTop:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc"}}>Checklist automÃ¡tico</div><span className="cxp-muted" style={{fontSize:10}}>{progress}%</span></div><div className="cxp-progress" style={{marginBottom:10}}><div style={{width:`${progress}%`}}/></div>{row.checklist.map(item=><label key={item.id} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 0",fontSize:12,cursor:"pointer"}}><input type="checkbox" checked={item.done} onChange={()=>onToggleChecklist(row.id,item.id)}/><span style={{textDecoration:item.done?"line-through":"none",color:item.done?"#6f5a95":"#e2d0ff"}}>{item.label}</span></label>)}</div>
    {row.status==="Publicado"&&<div className="cxp-card" style={{padding:12,marginTop:12}}><div className="cxp-title" style={{fontSize:11,color:"#86efac",marginBottom:10}}>Performance orgÃ¢nica</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{(["views","likes","shares","saves","followers_gained"] as (keyof Row)[]).map(k=><div key={String(k)}><div className="cxp-label">{{views:"Views",likes:"Curtidas",shares:"Compart.",saves:"Salvos",followers_gained:"Seguidores"}[k as string]}</div><input className="cxp-input" type="number" min="0" value={Number(row[k]||0)} onChange={e=>onUpdate(row.id,k,Number(e.target.value))}/></div>)}</div></div>}
    <div className="cxp-card" style={{padding:12,marginTop:12}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc",marginBottom:8}}>HistÃ³rico</div>{row.historico.length===0?<div className="cxp-muted" style={{fontSize:11}}>Nenhuma alteraÃ§Ã£o importante registrada ainda.</div>:row.historico.slice().reverse().slice(0,30).map((h,i)=><div key={`${h.at}-${i}`} style={{borderTop:i?"1px solid #211042":"none",padding:"7px 0"}}><div style={{fontSize:11}}><strong>{h.actor}</strong> alterou {prettyField(h.field)} <span className="cxp-muted">{h.from||"â€”"}</span> â†’ <span style={{color:"#c084fc"}}>{h.to||"â€”"}</span></div><div className="cxp-muted" style={{fontSize:9,marginTop:2}}>{humanDateTime(h.at)}</div></div>)}</div>
...
// â”€â”€â”€ Templates / recorrÃªncia / adaptaÃ§Ã£o â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TemplateModal({ onClose, onCreate }: { onClose:()=>void; onCreate:(t:TemplateDef)=>void }) {
  return <><div className="cxp-overlay" onClick={onClose}/><div className="cxp-modal"><SectionTitle eyebrow="Novo conteÃºdo">Criar a partir de modelo</SectionTitle><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:9}}>{TEMPLATES.map(t=><button key={t.id} onClick={()=>onCreate(t)} className="cxp-card" style={{padding:14,textAlign:"left",cursor:"pointer",color:"#e2d0ff"}}><div style={{fontSize:24,marginBottom:8}}>{t.icon}</div><div className="cxp-title" style={{fontSize:11,color:"#c084fc"}}>{t.label}</div><div className="cxp-muted" style={{fontSize:10,marginTop:4}}>{t.rede} Â· {t.formato}</div></button>)}</div><div style={{textAlign:"right",marginTop:12}}><button className="cxp-btn" onClick={onClose}>Cancelar</button></div></div></>;
}
function RecurrenceModal({ mes, onClose, onGenerate }: { mes:number;onClose:()=>void;onGenerate:(weekdays:number[],template:TemplateDef)=>void }) {
  ...
  return <><div className="cxp-overlay" onClick={onClose}/><div className="cxp-modal"><SectionTitle eyebrow={`RecorrÃªncia Â· ${MONTHS[mes]}`}>Gerar calendÃ¡rio automÃ¡tico</SectionTitle><div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}><button className="cxp-btn" onClick={()=>preset("twice")}>TerÃ§a + quinta</button><button className="cxp-btn" onClick={()=>preset("tt")}>3 extras TikTok / semana</button><button className="cxp-btn" onClick={()=>preset("daily")}>Story diÃ¡rio</button></div><div className="cxp-label">Modelo</div><select className="cxp-input" value={templateId} onChange={e=>setTemplateId(e.target.value)}>{TEMPLATES.map(t=><option value={t.id} key={t.id}>{t.label}</option>)}</select><div className="cxp-label" style={{marginTop:12}}>Dias da semana</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{WEEKDAYS.map((w,i)=><button key={w} className={`cxp-chip ${days.includes(i)?"active":""}`} onClick={()=>toggle(i)}>{w}</button>)}</div><div className="cxp-muted" style={{fontSize:11,marginTop:12}}>O sistema criarÃ¡ uma postagem em todos os dias selecionados deste mÃªs. Depois vocÃª pode arrastar datas normalmente.</div><div style={{display:"flex",justifyContent:"flex-end",gap:7,marginTop:14}}><button className="cxp-btn" onClick={onClose}>Cancelar</button><button className="cxp-btn primary" disabled={!days.length} onClick={()=>onGenerate(days,template)}>Gerar recorrÃªncia</button></div></div></>;
}
function AdaptModal({ row, onClose, onCreate }: { row:Row;onClose:()=>void;onCreate:(network:string,format:string)=>void }) {
  ...
  return <><div className="cxp-overlay" onClick={onClose}/><div className="cxp-modal"><SectionTitle eyebrow="Reaproveitar conteÃºdo">Adaptar para outra rede</SectionTitle><div className="cxp-muted" style={{fontSize:12,marginBottom:12}}>{row.tema||row.postagem}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}><div><div className="cxp-label">Rede destino</div><select className="cxp-input" value={network} onChange={e=>setNetwork(e.target.value)}>{REDE_OPTIONS.filter(r=>r!=="Todos").map(r=><option key={r}>{r}</option>)}</select></div><div><div className="cxp-label">Formato destino</div><select className="cxp-input" value={format} onChange={e=>setFormat(e.target.value)}>{FORMATO_OPTIONS.filter(f=>f!==EXTRA_TIKTOK_FORMAT).map(f=><option key={f}>{f}</option>)}</select></div></div><div className="cxp-muted" style={{fontSize:11,marginTop:12}}>Tema, hook, roteiro, arquivos e briefing serÃ£o copiados. A nova versÃ£o volta para â€œIdeiaâ€ com checklist novo.</div><div style={{display:"flex",justifyContent:"flex-end",gap:7,marginTop:14}}><button className="cxp-btn" onClick={onClose}>Cancelar</button><button className="cxp-btn primary" onClick={()=>onCreate(network,format)}>Criar adaptaÃ§Ã£o</button></div></div></>;
}

// â”€â”€â”€ Produtividade e performance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ProductivityView({ rows }: { rows:Row[] }) {
  ...
  return <div><SectionTitle eyebrow="O que estÃ¡ saindo do papel">Produtividade editorial</SectionTitle><div className="cxp-grid-kpis" style={{marginBottom:14}}>{[["Planejados",active.length,"#c084fc"],["Publicados",published.length,"#86efac"],["No prazo",`${onTimePct}%`,"#6ee7b7"],["Atrasados",overdue.length,"#fca5a5"]].map(([l,v,c])=><div className="cxp-card cxp-kpi" key={String(l)}><div className="cxp-label">{l}</div><strong style={{color:String(c)}}>{v}</strong></div>)}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc",marginBottom:10}}>Publicados por rede</div>{byNetwork.length?byNetwork.map(x=><div key={x.label} style={{display:"grid",gridTemplateColumns:"95px 1fr 30px",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:11}}>{REDE_ICONS[x.label]} {x.label}</span><div className="cxp-progress"><div style={{width:`${x.val/maxBar*100}%`}}/></div><span className="cxp-muted" style={{fontSize:10,textAlign:"right"}}>{x.val}</span></div>):<div className="cxp-muted" style={{fontSize:11}}>Sem publicaÃ§Ãµes ainda.</div>}</div><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc",marginBottom:10}}>Publicados por formato</div>{byFormat.length?byFormat.map(x=><div key={x.label} style={{display:"grid",gridTemplateColumns:"110px 1fr 30px",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:11}}>{x.label}</span><div className="cxp-progress"><div style={{width:`${x.val/maxBar*100}%`}}/></div><span className="cxp-muted" style={{fontSize:10,textAlign:"right"}}>{x.val}</span></div>):<div className="cxp-muted" style={{fontSize:11}}>Sem dados ainda.</div>}</div><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#c084fc",marginBottom:10}}>ProduÃ§Ã£o semanal</div>{weekly.length?weekly.map(x=><div key={x.label} style={{borderTop:"1px solid #211042",padding:"8px 0",display:"grid",gridTemplateColumns:"70px 1fr",gap:8}}><span style={{fontSize:10}}>{x.label}</span><span className="cxp-muted" style={{fontSize:10}}>{x.published} publicados / {x.planned} planejados</span></div>):<div className="cxp-muted" style={{fontSize:11}}>Sem conteÃºdo datado neste mÃªs.</div>}</div></div><SectionTitle eyebrow="Leitura do orgÃ¢nico" right={<span className="cxp-muted" style={{fontSize:10}}>{withPerf.length} posts com mÃ©tricas</span>}>Performance do conteÃºdo</SectionTitle><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#86efac",marginBottom:10}}>Formato x desempenho</div>{perfFormat.length?perfFormat.map(x=><div key={x.label} style={{borderTop:"1px solid #211042",padding:"8px 0",display:"flex",justifyContent:"space-between",gap:10}}><span style={{fontSize:11}}>{x.label}</span><span style={{fontSize:10,color:"#c084fc"}}>{x.avg.toLocaleString("pt-BR")} views Â· {x.eng.toLocaleString("pt-BR")} interaÃ§Ãµes</span></div>):<div className="cxp-muted" style={{fontSize:11}}>Preencha as mÃ©tricas nos posts publicados para comeÃ§ar a comparar formatos.</div>}</div><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#86efac",marginBottom:10}}>Top conteÃºdos / hooks</div>{top.length?top.map((r,i)=><div key={r.id} style={{borderTop:i?"1px solid #211042":"none",padding:"8px 0"}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><span style={{fontSize:11,fontWeight:700}}>{r.tema||r.postagem}</span><span style={{fontSize:10,color:"#c084fc"}}>{r.views.toLocaleString("pt-BR")}</span></div><div className="cxp-muted" style={{fontSize:9,marginTop:2}}>{r.hook?`Hook: ${r.hook}`:`${r.formato} Â· ${r.rede}`}</div></div>):<div className="cxp-muted" style={{fontSize:11}}>Ainda nÃ£o hÃ¡ mÃ©tricas orgÃ¢nicas suficientes.</div>}</div><div className="cxp-card" style={{padding:14}}><div className="cxp-title" style={{fontSize:11,color:"#86efac",marginBottom:10}}>Tema x desempenho</div>{perfTheme.length?perfTheme.map((x,i)=><div key={`${x.label}-${i}`} style={{borderTop:i?"1px solid #211042":"none",padding:"8px 0",display:"flex",justifyContent:"space-between",gap:10}}><span style={{fontSize:11,overflow:"hidden",textOverflow:"ellipsis"}}>{x.label}</span><span style={{fontSize:10,color:"#c084fc",whiteSpace:"nowrap"}}>{x.avg.toLocaleString("pt-BR")} views Â· {x.count} post{x.count!==1?"s":""}</span></div>):<div className="cxp-muted" style={{fontSize:11}}>Repita temas e preencha views para enxergar padrÃµes reais.</div>}</div></div></div>;
}

// â”€â”€â”€ Leads / CRM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function LeadsView({ isMobile, openNewNonce = 0 }: { isMobile:boolean; openNewNonce?:number }) {
  ...
  return <div><SectionTitle eyebrow="Mini CRM" right={<button className="cxp-btn primary" onClick={()=>setShowNew(true)}>ï¼‹ Novo lead</button>}>Leads & clientes</SectionTitle>{error&&<div className="cxp-card" style={{padding:10,borderColor:"#7f1d1d",color:"#fca5a5",fontSize:11,marginBottom:10}}>{error}</div>}<div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:7,marginBottom:10}}>{LEAD_STATUS_OPTIONS.map(s=>{const c=LEAD_STATUS_COLORS[s];const n=leads.filter(l=>l.status===s).length;return <button key={s} onClick={()=>setFilterStatus(filterStatus===s?"Todos":s)} style={{background:c.bg,border:`1px solid ${filterStatus===s?"#fff":c.border}`,borderRadius:9,padding:9,cursor:"pointer",color:c.text}}><div style={{font:"900 18px 'Cinzel',serif"}}>{n}</div><div style={{font:"700 8px 'Cinzel',serif"}}>{s}</div></button>})}</div><div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(3,1fr)":"repeat(5,1fr)",gap:7,marginBottom:10}}>{LEAD_CHANNELS.map(ch=>{const items=channelLeads(ch.chaves);return <button key={ch.label} onClick={()=>setExpandedChannel(expandedChannel===ch.label?null:ch.label)} className="cxp-card" style={{padding:8,cursor:"pointer",color:"#e2d0ff"}}><img src={ch.icon} alt="" style={{width:30,height:30,objectFit:"contain"}}/><div style={{font:"900 17px 'Cinzel',serif"}}>{items.length}</div><div className="cxp-muted" style={{fontSize:8}}>{ch.label}</div></button>})}</div>{expandedChannel&&(()=>{const ch=LEAD_CHANNELS.find(x=>x.label===expandedChannel)!;const items=channelLeads(ch.chaves);return <div className="cxp-card" style={{padding:10,marginBottom:10}}><div className="cxp-label">{ch.label} Â· funil</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{LEAD_STATUS_OPTIONS.map(s=><span key={s} className="cxp-chip">{s}: {items.filter(l=>l.status===s).length}</span>)}</div></div>})()}<div style={{display:"flex",gap:7,marginBottom:10,flexWrap:"wrap"}}><input className="cxp-input" style={{flex:1,minWidth:220}} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar nome, contato, origem, campanhaâ€¦"/><select className="cxp-input" style={{width:180}} value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}><option>Todos</option>{LEAD_STATUS_OPTIONS.map(s=><option key={s}>{s}</option>)}</select><button className="cxp-btn" onClick={load}>âŸ³</button></div>{loading?<div style={{padding:50,display:"flex",justifyContent:"center"}}><Spinner/></div>:<div style={{display:"grid",gap:8}}>{filtered.map(lead=>{const c=LEAD_STATUS_COLORS[lead.status]||LEAD_STATUS_COLORS["Novo lead"];const open=expanded===lead.id;return <div key={lead.id} className="cxp-card" style={{borderLeft:`4px solid ${c.border}`,overflow:"hidden"}}><div onClick={()=>setExpanded(open?null:lead.id)} style={{padding:11,display:"flex",gap:8,alignItems:"center",cursor:"pointer"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700}}>{lead.nome||"â€”"}</div><div className="cxp-muted" style={{fontSize:10}}>{lead.whatsapp_discord||"sem contato"} Â· {lead.origem||lead.utm_source||"origem nÃ£o informada"}</div></div><select value={lead.status} onClick={e=>e.stopPropagation()} onChange={e=>patch(lead.id,{status:e.target.value})} style={{...inputStyle,width:"auto",background:c.bg,color:c.text,borderColor:c.border,fontSize:10}}>{LEAD_STATUS_OPTIONS.map(s=><option key={s}>{s}</option>)}</select></div>{open&&<div style={{padding:"0 11px 12px",borderTop:"1px solid #211042"}}><div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,1fr)",gap:8,marginTop:10}}>{[
          ["PrÃ³xima aÃ§Ã£o","proxima_acao","text"],["Follow-up","follow_up_date","date"],["ResponsÃ¡vel","responsavel","text"],["Ãšltimo contato","ultimo_contato","date"],["AnotaÃ§Ã£o rÃ¡pida","anotacao_rapida","text"],["Origem","origem","text"],["UTM Source","utm_source","text"],["UTM Campaign","utm_campaign","text"],["Influencer","influencer_codigo","text"]
        ].map(([label,key,type])=><div key={key}><div className="cxp-label">{label}</div><input className="cxp-input" type={type} defaultValue={String((lead as any)[key]||"")} onBlur={e=>patch(lead.id,{[key]:e.target.value} as any)}/></div>)}</div><div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:8,marginTop:10}}>{[["Idade",lead.idade],["Tempo RPG",lead.tempo_rpg],["Sistemas",lead.sistemas_desejados],["Melhor dia",lead.melhor_dia]].map(([l,v])=><div key={l}><div className="cxp-label">{l}</div><div style={{fontSize:11}}>{v||"â€”"}</div></div>)}</div></div>}</div>})}</div>}{showNew&&<><div className="cxp-overlay" onClick={()=>setShowNew(false)}/><div className="cxp-modal"><SectionTitle>Novo lead manual</SectionTitle><div style={{display:"grid",gap:8}}><input className="cxp-input" placeholder="Nome" value={newLead.nome} onChange={e=>setNewLead(p=>({...p,nome:e.target.value}))}/><input className="cxp-input" placeholder="WhatsApp / Discord" value={newLead.whatsapp_discord} onChange={e=>setNewLead(p=>({...p,whatsapp_discord:e.target.value}))}/><input className="cxp-input" placeholder="Origem" value={newLead.origem} onChange={e=>setNewLead(p=>({...p,origem:e.target.value}))}/></div><div style={{display:"flex",justifyContent:"flex-end",gap:7,marginTop:12}}><button className="cxp-btn" onClick={()=>setShowNew(false)}>Cancelar</button><button className="cxp-btn primary" onClick={create}>Criar</button></div></div></>}</div>;
}
...
function InfluencersView({ isMobile }: { isMobile:boolean }) {
  ...
  return <div><SectionTitle eyebrow="AquisiÃ§Ã£o por parceiro">Influencers & links</SectionTitle><div className="cxp-card" style={{padding:12,marginBottom:12}}><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"end"}}><div style={{flex:1,minWidth:180}}><div className="cxp-label">Nome</div><input className="cxp-input" value={nome} onChange={e=>{setNome(e.target.value);if(!codigo)setCodigo(slugifyCodigo(e.target.value))}}/></div><div style={{width:isMobile?"100%":180}}><div className="cxp-label">CÃ³digo</div><input className="cxp-input" value={codigo} onChange={e=>setCodigo(slugifyCodigo(e.target.value))}/></div><button className="cxp-btn primary" disabled={saving} onClick={create}>{saving?"Criandoâ€¦":"Criar link"}</button></div>{error&&<div style={{color:"#fca5a5",fontSize:11,marginTop:8}}>{error}</div>}</div>{loading?<div style={{padding:50,display:"flex",justifyContent:"center"}}><Spinner/></div>:<div style={{display:"grid",gap:9}}>{ranking.map((inf,idx)=>{const li=leadsFor(inf.codigo);const qualified=li.filter(l=>["Em contato","Lista de espera","Mesa alocada"].includes(l.status));const final=li.filter(l=>l.status==="Mesa alocada");const finalRate=inf.clicks?final.length/inf.clicks*100:0;const link=`${window.location.origin}/?ref=${inf.codigo}`;return <div key={inf.id} className="cxp-card" style={{padding:13,borderLeft:`4px solid ${inf.ativo?"#7c3aed":"#4b5563"}`,opacity:inf.ativo?1:.65}}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{font:"900 16px 'Cinzel',serif",color:idx===0&&li.length?"#fbbf24":"#6f5a95"}}>{idx===0&&li.length?"ðŸ†":`#${idx+1}`}</span><div style={{flex:1,fontWeight:700}}>{inf.nome}</div><button className={`cxp-btn ${inf.ativo?"green":""}`} onClick={()=>{setInfluencers(p=>p.map(x=>x.id===inf.id?{...x,ativo:!x.ativo}:x));dbUpdateInfluencer(inf.id,{ativo:!inf.ativo}).catch(()=>load())}}>{inf.ativo?"Ativo":"Pausado"}</button><button className="cxp-btn danger" onClick={async()=>{if(window.confirm(`Excluir ${inf.nome}?`)){await dbDeleteInfluencer(inf.id);load()}}}>âœ•</button></div><div style={{display:"flex",gap:6,marginTop:9}}><input className="cxp-input" readOnly value={link}/><button className="cxp-btn" onClick={()=>{navigator.clipboard.writeText(link);setCopied(inf.codigo);setTimeout(()=>setCopied(null),1200)}}>{copied===inf.codigo?"Copiado":"Copiar"}</button></div><div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(5,1fr)",gap:7,marginTop:9}}>{[["Cliques",inf.clicks,"#93c5fd"],["Leads",li.length,"#c084fc"],["Qualificados",qualified.length,"#fcd34d"],["Mesas",final.length,"#86efac"],["Conv. final",`${finalRate.toFixed(1)}%`,"#6ee7b7"]].map(([l,v,c])=><div key={String(l)} style={{background:"#0d0720",borderRadius:8,padding:8,textAlign:"center"}}><div style={{font:"900 17px 'Cinzel',serif",color:String(c)}}>{v}</div><div className="cxp-label" style={{margin:0}}>{l}</div></div>)}</div></div>})}</div>}</div>;
}

// â”€â”€â”€ Command palette â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function CommandPalette({ open, onClose, onNewPost, onExtraTikTok, onToday, onLeads, onInfluencers, onOpenPost }: { open:boolean;onClose:()=>void;onNewPost:()=>void;onExtraTikTok:()=>void;onToday:()=>void;onLeads:()=>void;onInfluencers:()=>void;onOpenPost:(r:Row)=>void }) {
  ...
  return <><div className="cxp-overlay" style={{zIndex:1199}} onClick={onClose}/><div className="cxp-command"><input id="cxp-command-input" value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar tudo ou executar um comandoâ€¦" style={{width:"100%",background:"#0d0720",border:"none",borderBottom:"1px solid #3d246f",padding:"16px",color:"#e2d0ff",outline:"none",fontSize:14}}/>{loading&&<div style={{padding:15,display:"flex",justifyContent:"center"}}><Spinner/></div>}{!query&&!loading&&<>{[["ï¼‹","Nova postagem",onNewPost],["ðŸŽµ","VÃ­deo extra TikTok",onExtraTikTok],["â˜€","Ir para Central de Hoje",onToday],["ðŸ‘¥","Novo lead / abrir CRM",onLeads],["ðŸ”—","Novo influencer",onInfluencers]].map(([icon,label,fn])=><div key={String(label)} className="cxp-command-row" onClick={()=>run(fn as ()=>void)}><span>{icon as string}</span><span style={{fontSize:12}}>{label as string}</span></div>)}</>}{query&&!loading&&<>{postResults.map(r=><div key={r.id} className="cxp-command-row" onClick={()=>run(()=>onOpenPost(r))}><span>{REDE_ICONS[r.rede]||"ðŸ“„"}</span><div><div style={{fontSize:12}}>{r.tema||r.postagem}</div><div className="cxp-muted" style={{fontSize:9}}>ConteÃºdo Â· {r.data||"sem data"} Â· {r.formato}</div></div></div>)}{leadResults.map(l=><div key={l.id} className="cxp-command-row" onClick={()=>run(onLeads)}><span>ðŸ‘¥</span><div><div style={{fontSize:12}}>{l.nome}</div><div className="cxp-muted" style={{fontSize:9}}>Lead Â· {l.whatsapp_discord||"sem contato"}</div></div></div>)}{infResults.map(i=><div key={i.id} className="cxp-command-row" onClick={()=>run(onInfluencers)}><span>ðŸ”—</span><div><div style={{fontSize:12}}>{i.nome}</div><div className="cxp-muted" style={{fontSize:9}}>Influencer Â· {i.codigo}</div></div></div>)}{!postResults.length&&!leadResults.length&&!infResults.length&&<div className="cxp-muted" style={{padding:18,textAlign:"center",fontSize:11}}>Nada encontrado.</div>}</>}</div></>;
}
...
function Dashboard({ onVoltar }: { onVoltar:()=>void }) {
  ...
  const adapt=async(source:Row,network:string,format:string)=>{const row=makeRow(rowsRef.current.length+1,mes,"",{...source,id:undefined as any,postagem:`${source.postagem} Â· ${network}`,rede:network,formato:format,status:"Ideia",checklist:makeChecklist(format),historico:[makeHistory("adaptaÃ§Ã£o",source.rede,network)],published_at:null,views:0,likes:0,shares:0,saves:0,followers_gained:0});setRowsSafe(p=>{const i=p.findIndex(r=>r.id===source.id);const next=[...p];next.splice(i+1,0,row);return next});setAdaptRow(null);await dbUpsert(row);setSelectedPostId(row.id)};
...
export default function App() {
  ...
  const login=<div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0d0720,#1a0d3a)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{width:"min(360px,100%)",background:"#100722",border:"1px solid #4a2a8a",borderRadius:16,padding:28,textAlign:"center"}}><img src="/icons/criandoxp.png" alt="" style={{width:64,height:64,objectFit:"contain",marginBottom:12}}/><div style={{font:"900 17px 'Cinzel',serif",color:"#c084fc",marginBottom:6}}>Ãrea Restrita</div><div style={{font:"12px 'Lato',sans-serif",color:"#6f5a95",marginBottom:18}}>Criando XP Â· Central Operacional</div><input type="password" value={senha} onChange={e=>setSenha(e.target.value)} onKeyDown={e=>e.key==="Enter"&&entrar()} placeholder="Senha" style={{...inputStyle,padding:12,fontSize:14}}/>{erro&&<div style={{color:"#fca5a5",fontSize:11,marginTop:7}}>{erro}</div>}<button onClick={entrar} disabled={entrando} style={{width:"100%",marginTop:10,background:"linear-gradient(135deg,#6d28d9,#9333ea)",color:"#fff",border:0,borderRadius:9,padding:12,font:"700 12px 'Cinzel',serif",cursor:"pointer",opacity:entrando?.6:1}}>{entrando?"Entrandoâ€¦":"Entrar"}</button></div></div>;
