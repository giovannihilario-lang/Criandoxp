    ...
    <div>
      <style>{`
        @media (max-width: 600px) {
          .trafego-charts-row { grid-template-columns: 1fr !important; }
          .trafego-kpis { grid-template-columns: 1fr 1fr !important; }
          .trafego-campaigns { grid-template-columns: 1fr !important; }
        }
      `}</style>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div className="trafego-kpis" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          {[
            ...
            <div key={k.label} style={{
              ...
              border: "1px solid #4a2a8a", borderTop: `2px solid ${k.color}`,
              borderRadius: 12, padding: "16px 14px",
              animation: "glow 3s ease-in-out infinite",
            }}>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: "#a78bfa", marginBottom: 6, letterSpacing: 1 }}>{k.label}</div>
              <div style={{ fontSize: 26, fontWeight: 900, color: k.color, fontFamily: "'Cinzel', serif", letterSpacing: -1 }}>{k.val}</div>
      ...
      <div className="trafego-charts-row" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16, marginBottom: 28 }}>

        {/* Bar chart — horizontal on mobile */}
        {mysticCard(
          <>
            ...
            <div style={{ height: 280 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={CHART_DATA} layout="vertical" margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
                  <XAxis type="number" tick={{ fill: "#7a6a9a", fontSize: 9, fontFamily: "'Lato', sans-serif" }} tickFormatter={v => v >= 1000 ? (v/1000).toFixed(0)+"k" : v} />
                  <YAxis type="category" dataKey="name" width={110} tick={{ fill: "#9d8bbf", fontSize: 10, fontFamily: "'Lato', sans-serif" }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="vizTotal" name="Visualizações" radius={[0,4,4,0]}>
                    {CHART_DATA.map((_, i) => <Cell key={i} fill={`rgba(162,89,255,${0.9 - i*0.08})`} />)}
                  </Bar>
                  <Bar dataKey="vizUnicos" name="Visualizadores" radius={[0,4,4,0]}>
        ...
        {mysticCard(
          <>
            ...
            <div style={{ fontSize: 10, color: "#9d8bbf", fontFamily: "'Lato', sans-serif", marginBottom: 16 }}>8,1 mil interações</div>
            {AUDIENCE_DATA.map(a => (
              <div key={a.faixa} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: "#9d8bbf", width: 38, flexShrink: 0 }}>{a.faixa}</div>
                <div style={{ flex: 1, height: 20, background: "#0d0720", borderRadius: 4, overflow: "hidden", display: "flex" }}>
                  <div style={{ width: `${a.homens}%`, background: "rgba(162,89,255,0.85)", height: "100%", borderRadius: "4px 0 0 4px" }} />
                  <div style={{ width: `${a.mulheres}%`, background: "rgba(240,171,252,0.7)", height: "100%" }} />
                </div>
                <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 10, color: "#9d8bbf", width: 28, textAlign: "right" }}>{a.total}%</div>
              </div>
            ))}
            <div style={{ display: "flex", gap: 14, marginTop: 12 }}>
              {[["rgba(162,89,255,0.85)","Homens"],["rgba(240,171,252,0.7)","Mulheres"]].map(([bg, label]) => (
                <div key={label as string} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <span style={{ fontSize: 10, color: "#9d8bbf", fontFamily: "'Lato', sans-serif" }}>{label}</span>
                </div>
              ))}
            </div>
          </>
      ...
      <div>
        ...
        <div className="trafego-campaigns" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
          {CAMPAIGNS.map((c, idx) => (
            <div key={c.id} style={{
              background: "linear-gradient(135deg, #1a0d3a, #110828)",
            ...
            >
              {c.best && (
                <div style={{ position: "absolute", top: 12, right: 12, background: "rgba(162,89,255,0.15)", border: "1px solid rgba(162,89,255,0.4)", color: "#c084fc", fontFamily: "'Cinzel', serif", fontSize: 8, letterSpacing: 1, textTransform: "uppercase", padding: "3px 9px", borderRadius: 20 }}>✦ Mais visitas</div>
              )}
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: 9, color: "#9d8bbf", marginBottom: 6, letterSpacing: 1 }}>Campanha {String(idx + 1).padStart(2, "0")}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#e2d0ff", marginBottom: 3, lineHeight: 1.3, paddingRight: c.best ? 80 : 0, fontFamily: "'Cinzel', serif" }}>{c.nome}</div>
              <div style={{ fontSize: 10, color: "#9d8bbf", marginBottom: 14, fontFamily: "'Lato', sans-serif" }}>{c.periodo}</div>
              <div style={{ borderTop: "1px solid #2d1b69", paddingTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Cinzel', serif", letterSpacing: 1, marginBottom: 3 }}>VISUALIZAÇÕES</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: "#e2d0ff" }}>{c.vizTotal.toLocaleString("pt-BR")}</div>
                </div>
                <div>
                  <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Cinzel', serif", letterSpacing: 1, marginBottom: 3 }}>VISUALIZADORES</div>
                ...
                <div style={{ gridColumn: "1 / -1", borderTop: "1px solid #2d1b69", paddingTop: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Cinzel', serif", letterSpacing: 1, marginBottom: 3 }}>RESULTADO</div>
                    <div style={{ fontSize: 22, fontWeight: 900, color: "#c084fc", fontFamily: "'Cinzel', serif" }}>{c.resultado}</div>
                    <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Lato', sans-serif" }}>{c.tipoResultado}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Cinzel', serif", letterSpacing: 1, marginBottom: 3 }}>TAXA</div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#a78bfa", fontFamily: "'Cinzel', serif" }}>
                      {((c.resultado / c.vizTotal) * 100).toFixed(2)}%
                    </div>
                    <div style={{ fontSize: 9, color: "#9d8bbf", fontFamily: "'Lato', sans-serif" }}>conversão</div>
                  </div>
                </div>
              </div>
            </div>
