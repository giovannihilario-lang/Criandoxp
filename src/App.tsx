...
async function dbLoadCentralRows(): Promise<Row[]> {
  const year = new Date().getFullYear();
  const [dated, backlog] = await Promise.all([
    supabase.from("postagens").select("*").gte("data_iso", `${year}-01-01`).lte("data_iso", `${year}-12-31`).order("data_iso", { ascending: true }),
    supabase.from("postagens").select("*").is("data_iso", null).order("created_at", { ascending: false }),
  ]);
...
/* ── Identidade Criando XP restaurada ─────────────────────────────── */
:root{
  --ink:#0d0720;--ink-2:#110828;--panel:#1a0d3a;--panel-2:#160b31;--panel-3:#24114f;
  --line:#2d1b69;--line-strong:#4a2a8a;--text:#e2d0ff;--muted:#a08abf;
  --violet:#7c3aed;--violet-2:#a855f7;--violet-3:#c084fc;--gold:#e9c46a;
  --shadow:0 20px 65px rgba(4,1,14,.46);--radius:14px;--radius-sm:9px;
}
body{background:#0d0720;color:var(--text);font-family:'Lato',system-ui,sans-serif}
.cxp-app{background:
 radial-gradient(circle at 84% -8%,rgba(168,85,247,.24),transparent 31%),
 radial-gradient(circle at 9% 27%,rgba(124,58,237,.12),transparent 29%),
 linear-gradient(rgba(124,58,237,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(124,58,237,.055) 1px,transparent 1px),
 linear-gradient(135deg,#0d0720 0%,#150a30 42%,#0d0720 100%);background-size:auto,auto,48px 48px,48px 48px,auto}
.cxp-sidebar{background:linear-gradient(180deg,rgba(17,8,40,.99),rgba(13,7,32,.99));border-right-color:#2d1b69;box-shadow:12px 0 42px rgba(9,2,25,.2)}
.cxp-brand{padding-bottom:18px;margin-bottom:3px;border-bottom:1px solid rgba(74,42,138,.45)}
.cxp-brand-logo{width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#c084fc);border:1px solid rgba(232,121,249,.55);box-shadow:0 0 0 5px rgba(124,58,237,.08),0 12px 34px rgba(124,58,237,.32)}
.cxp-brand-logo img{width:38px;height:38px}.cxp-brand-title,.cxp-topbar-name{background:linear-gradient(90deg,#e879f9,#c084fc,#818cf8);-webkit-background-clip:text;background-clip:text;color:transparent;font-weight:900;letter-spacing:.07em}
.cxp-brand-title{font-size:16px}.cxp-brand-sub,.cxp-topbar-eyebrow,.cxp-side-label,.cxp-section-eyebrow{color:#7c3aed}
.cxp-side-item{color:#a78bc7;border:1px solid transparent;font-family:'Cinzel',serif;font-size:11px;letter-spacing:.025em}.cxp-side-item:hover{background:#1a0d3a;color:#e2d0ff;border-color:#2d1b69}.cxp-side-item.active{background:linear-gradient(135deg,#3b1d70,#221047);color:#fff;border-color:#6b3fa0;box-shadow:inset 3px 0 #c084fc,0 10px 22px rgba(7,2,20,.18)}.cxp-side-icon{background:#170b34;border:1px solid #2d1b69;color:#c084fc}
.cxp-topbar{background:linear-gradient(180deg,rgba(13,7,32,.99),rgba(13,7,32,.88),transparent);border-bottom:1px solid rgba(74,42,138,.24)}
.cxp-panel{background:linear-gradient(135deg,rgba(26,13,58,.97),rgba(17,8,40,.98));border-color:#2d1b69;box-shadow:0 1px 0 rgba(192,132,252,.03) inset,0 14px 40px rgba(4,1,14,.12)}
.cxp-panel::before{background:linear-gradient(90deg,#7c3aed,transparent)}.cxp-panel::after{background:linear-gradient(#7c3aed,transparent)}
.cxp-card{background:linear-gradient(135deg,#1a0d3a,#110828);border-color:#2d1b69;border-radius:12px}.cxp-card.interactive:hover{border-color:#7c3aed;box-shadow:0 12px 38px rgba(124,58,237,.15)}
.cxp-btn{background:#1a0d3a;color:#c9a0f5;border-color:#4a2a8a;border-radius:9px;font-family:'Cinzel',serif;font-weight:700;letter-spacing:.015em}.cxp-btn:hover{background:#24114f;border-color:#7c3aed;color:#fff}.cxp-btn.primary{background:linear-gradient(135deg,#4a2a8a,#7c3aed,#a855f7);box-shadow:0 10px 26px rgba(124,58,237,.28)}.cxp-btn.ghost{background:transparent}.cxp-btn.green{background:rgba(22,163,74,.12)}
.cxp-input,.cxp-date-trigger{background:#110828;color:#e2d0ff;border-color:#4a2a8a;border-radius:9px;font-family:'Lato',sans-serif}.cxp-input:focus,.cxp-date-trigger:focus-visible{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,.16)}.cxp-input::placeholder{color:#76559e}
.cxp-label{color:#8d6ab7;font-family:'Cinzel',serif}.cxp-muted{color:#a08abf}.cxp-section-title,.cxp-title{color:#eadfff}.cxp-section-title{font-weight:900}
.cxp-chip{background:#160a31;color:#bda0df;border-color:#3d226f;border-radius:8px;font-family:'Cinzel',serif}.cxp-chip.active{background:linear-gradient(135deg,#3b1d70,#24114f);border-color:#7c3aed;color:#fff}
.cxp-kpi{border-color:#2d1b69}.cxp-kpi .value{color:#e2d0ff}.cxp-kpi .hint{color:#8d6ab7}
.cxp-searchbar,.cxp-view-switch{background:#110828;border-color:#4a2a8a}.cxp-view-switch button.active{background:linear-gradient(135deg,#4a2a8a,#7c3aed)}
.cxp-modal,.cxp-drawer,.cxp-date-popover,.cxp-command,.cxp-file-modal{background:linear-gradient(160deg,#1a0d3a,#0d0720);border-color:#4a2a8a;box-shadow:0 36px 110px rgba(3,0,12,.7)}
.cxp-overlay{background:rgba(5,1,15,.76)}.cxp-drawer-section{border-color:#2d1b69}.cxp-drawer-footer{background:rgba(13,7,32,.96);border-color:#2d1b69}
.cxp-origin{padding:12px 9px!important;min-height:112px;display:flex!important;flex-direction:column;align-items:center;justify-content:center;gap:6px}.cxp-origin img{width:42px!important;height:42px!important;object-fit:contain;filter:drop-shadow(0 6px 12px rgba(0,0,0,.2))}.cxp-origin strong{font:900 21px 'Cinzel',serif;color:#e2d0ff}.cxp-origin-name{font:700 9px 'Cinzel',serif;color:#7c5ba5;letter-spacing:.05em}
.cxp-status-card{box-shadow:inset 0 0 0 1px rgba(255,255,255,.015)}
.cxp-lead-avatar{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;border:1px solid;flex:0 0 auto;font:900 14px 'Cinzel',serif}.cxp-lead-name{font:800 13px 'Cinzel',serif;color:#e2d0ff;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cxp-lead-meta{font:400 11px 'Lato',sans-serif;color:#8f71b3;margin-top:4px;display:block;overflow-wrap:anywhere}.cxp-lead-quick{display:flex;gap:5px;align-items:center;flex-wrap:wrap}.cxp-lead-contact-strip{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;background:#110828;border:1px solid #2d1b69;border-radius:10px;padding:11px;margin-bottom:11px}.cxp-lead-contact-strip strong{font:800 13px 'Cinzel',serif;display:block}.cxp-lead-contact-strip span{display:block;color:#a98cc9;font-size:11px;margin-top:3px}.cxp-lead-contact-actions{display:flex;gap:6px;flex-wrap:wrap}.cxp-inline-success{margin:-2px 0 10px;padding:8px 10px;border:1px solid rgba(22,163,74,.4);background:rgba(22,163,74,.1);border-radius:9px;color:#86efac;font:700 10px 'Cinzel',serif}
.cxp-urgency{display:inline-flex;align-items:center;gap:3px;border:1px solid;border-radius:6px;padding:2px 5px;font:900 8px 'Cinzel',serif;letter-spacing:.05em;vertical-align:middle;white-space:nowrap}.cxp-urgency.hoje,.cxp-urgency.atrasado{animation:pulseRing 1.6s ease-in-out infinite}.cxp-header-urgent{border:1px solid rgba(239,68,68,.42);background:rgba(127,29,29,.2);color:#fca5a5;border-radius:999px;padding:5px 9px;font:800 9px 'Cinzel',serif;white-space:nowrap}
.cxp-files{margin-top:8px;background:#110828;border:1px solid #2d1b69;border-radius:10px;padding:11px}.cxp-files-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.cxp-file-cover{position:relative;width:100%;height:118px;margin-top:9px;padding:0;overflow:hidden;border:1px solid #4a2a8a;border-radius:9px;background:linear-gradient(135deg,#160b31,#0d0720);cursor:pointer;color:#fff}.cxp-file-cover img{width:100%;height:100%;object-fit:cover;display:block;opacity:.76}.cxp-file-cover-placeholder{display:grid;place-items:center;width:100%;height:100%;font-size:28px;color:#7c3aed}.cxp-file-cover-overlay{position:absolute;inset:0;display:grid;place-items:center;background:rgba(5,1,15,.36);font:800 11px 'Cinzel',serif;opacity:0;transition:opacity .18s ease}.cxp-file-cover:hover .cxp-file-cover-overlay,.cxp-file-cover:focus-visible .cxp-file-cover-overlay{opacity:1}.cxp-file-count{position:absolute;right:7px;bottom:7px;background:#7c3aed;color:#fff;border-radius:999px;padding:3px 7px;font:800 9px 'Cinzel',serif}.cxp-file-list{display:grid;gap:5px;margin-top:9px}.cxp-file-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:7px;align-items:center;background:#160b31;border:1px solid #2d1b69;border-radius:8px;padding:6px}.cxp-file-row a{color:#c9a0f5;font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cxp-file-icon{font-size:15px}.cxp-file-modal{position:fixed;z-index:1801;left:50%;top:50%;transform:translate(-50%,-50%);width:min(960px,calc(100vw - 24px));max-height:92dvh;border:1px solid;overflow:auto;border-radius:14px;padding:15px}.cxp-file-nav,.cxp-file-modal-actions{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:8px 0}.cxp-file-nav span{font:800 10px 'Cinzel',serif;color:#c084fc}.cxp-file-frame-wrap{background:#080312;border:1px solid #2d1b69;border-radius:9px;overflow:hidden}.cxp-file-frame{display:block;width:100%;height:min(66vh,680px);border:0;background:#fff}
.cxp-bottom-nav{background:rgba(17,8,40,.97);border-color:#4a2a8a}.cxp-bottom-item{color:#76559e;font-family:'Cinzel',serif}.cxp-bottom-item.active{background:linear-gradient(135deg,rgba(74,42,138,.8),rgba(124,58,237,.55));color:#fff}.cxp-fab{border-color:#c084fc;background:linear-gradient(135deg,#7c3aed,#c084fc);box-shadow:0 16px 38px rgba(124,58,237,.45)}.cxp-mobile-top{background:rgba(13,7,32,.96);border-color:#2d1b69}
.cxp-status-overview{display:grid;grid-template-columns:repeat(8,minmax(78px,1fr));gap:7px;margin:0 0 11px}.cxp-status-summary{min-width:0;border:1px solid #2d1b69;border-radius:10px;background:#1a0d3a;color:#c9a0f5;padding:8px 7px;text-align:center;cursor:pointer;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease;opacity:.78}.cxp-status-summary:hover{transform:translateY(-1px);opacity:1}.cxp-status-summary.active{opacity:1;box-shadow:0 0 0 2px rgba(192,132,252,.22),0 10px 28px rgba(4,1,14,.18)}.cxp-status-summary strong{display:block;font:900 19px 'Cinzel',serif;line-height:1}.cxp-status-summary span{display:block;margin-top:5px;font:700 8px 'Cinzel',serif;letter-spacing:.03em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
/* Identidade Criando XP aplicada também às superfícies operacionais */
::-webkit-scrollbar-track{background:#0d0720}::-webkit-scrollbar-thumb{background:#4a2a8a}::-webkit-scrollbar-thumb:hover{background:#7c3aed}
.cxp-calendar-wrap,.cxp-week-column,.cxp-agenda-day,.cxp-agenda-item,.cxp-backlog,.cxp-idea,.cxp-kanban-col,.cxp-kanban-card,.cxp-table-wrap,.cxp-mobile-card,.cxp-metric,.cxp-state,.cxp-login-card{background:linear-gradient(145deg,#1a0d3a,#110828);border-color:#2d1b69}
.cxp-day{background:linear-gradient(145deg,rgba(26,13,58,.93),rgba(17,8,40,.96));border-color:#2d1b69}.cxp-day:hover{background:#221047;border-color:#7c3aed}.cxp-day.today{border-color:#a855f7;box-shadow:inset 0 0 0 1px rgba(168,85,247,.18),0 0 24px rgba(124,58,237,.08)}.cxp-day.drop{background:#2a1554;outline-color:#c084fc}.cxp-day-number{color:#a98cc9}.cxp-day-add{color:#8d6ab7}.cxp-day-add:hover{background:#321965;color:#fff}
.cxp-week-column.today{border-color:#a855f7;box-shadow:inset 0 0 0 1px rgba(168,85,247,.1)}.cxp-week-column-head{border-color:#2d1b69}.cxp-agenda-item:hover,.cxp-idea:hover,.cxp-idea:focus-visible,.cxp-kanban-card:hover{background:#24114f;border-color:#7c3aed}.cxp-agenda-date span{color:#8d6ab7}
.cxp-kanban-col{background:linear-gradient(180deg,#13092d,#0d0720)}.cxp-kanban-card{box-shadow:0 8px 22px rgba(4,1,14,.16)}.cxp-progress,.cxp-bar{background:#09041a}
.cxp-table-wrap{background:#110828}.cxp-table th{background:#1a0d3a;color:#8d6ab7;border-color:#2d1b69;font-family:'Cinzel',serif}.cxp-table td{border-color:rgba(74,42,138,.38)}.cxp-table tr:hover td{background:rgba(124,58,237,.07)}.cxp-table-sub,.cxp-mobile-card-meta{color:#a08abf}.cxp-mobile-card{background:linear-gradient(145deg,#1a0d3a,#110828)}
.cxp-searchbar kbd{background:#1a0d3a;border-color:#4a2a8a;color:#c9a0f5}.cxp-command{background:linear-gradient(160deg,#1a0d3a,#0d0720);border-color:#4a2a8a}.cxp-command-input{background:#110828;color:#e2d0ff;border-color:#2d1b69}.cxp-command-row{border-color:rgba(74,42,138,.34)}.cxp-command-row:hover,.cxp-command-row:focus-visible{background:#24114f}
.cxp-toast{background:#1a0d3a;border-color:#4a2a8a;color:#e2d0ff}.cxp-state{background:linear-gradient(135deg,#1a0d3a,#110828)}.cxp-state-mark{background:#2a1554;color:#d8b4fe}.cxp-state p,.cxp-help-grid p,.cxp-shortcuts dt,.cxp-loading-label{color:#a08abf}
.cxp-login-shell{background:radial-gradient(circle at 50% 8%,rgba(168,85,247,.22),transparent 34%),linear-gradient(135deg,#0d0720,#1a0d3a 52%,#0d0720)}.cxp-login-card{box-shadow:0 26px 80px rgba(3,0,12,.58)}.cxp-login-copy{color:#a08abf}.cxp-login-brand{width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#c084fc);border-color:rgba(232,121,249,.5);box-shadow:0 0 0 6px rgba(124,58,237,.08),0 14px 34px rgba(124,58,237,.26)}.cxp-login-brand img{width:46px;height:46px}
.cxp-tour-backdrop{background:rgba(5,1,15,.8)}.cxp-tour-card{background:linear-gradient(155deg,#1a0d3a,#110828);border-color:#4a2a8a}.cxp-tour-card p,.cxp-tour-top,.cxp-tour-name small{color:#a08abf}.cxp-tour-progress{background:#2d1b69}.cxp-tour-name input{background:#110828;border-color:#4a2a8a;color:#e2d0ff}
.cxp-skeleton-line,.cxp-skeleton-grid span{background:linear-gradient(90deg,#170b34,#2a1554,#170b34);background-size:220% 100%}
.cxp-date-day{color:#c9a0f5}.cxp-date-day:hover{background:#321965}.cxp-date-weekday{color:#8d6ab7}.cxp-flow-step{background:#110828;color:#8d6ab7;border-color:#2d1b69}.cxp-flow-step.current{background:linear-gradient(135deg,#4a2a8a,#7c3aed);border-color:#a855f7}.cxp-flow-step.done{background:#221047;color:#c9a0f5}
@media(max-width:920px){.cxp-status-overview{display:flex;overflow-x:auto;scroll-snap-type:x proximity;margin-left:-2px;margin-right:-2px;padding:2px 2px 8px}.cxp-status-summary{flex:0 0 100px;min-height:64px;scroll-snap-align:start}.cxp-status-summary strong{font-size:18px}.cxp-brand-logo{width:44px;height:44px}.cxp-lead-head{align-items:flex-start}.cxp-lead-quick{width:100%;padding:0 10px 10px}.cxp-lead-quick .cxp-input{flex:1}.cxp-origin{min-height:104px}.cxp-origin img{width:36px!important;height:36px!important}.cxp-file-modal{left:0;right:0;bottom:0;top:auto;transform:none;width:100vw;max-height:90dvh;border-radius:18px 18px 0 0;border-left:0;border-right:0;border-bottom:0}.cxp-file-frame{height:56vh}.cxp-header-urgent{display:none}.cxp-drawer-footer{grid-template-columns:1fr 1fr}.cxp-drawer-footer .danger{grid-column:1/-1}.cxp-lead-quick{justify-content:flex-start}}

.cxp-quick-create{width:min(620px,calc(100vw - 28px))}.cxp-quick-copy{margin:-2px 0 14px;color:#a08abf;font-size:11px;line-height:1.55}.cxp-quick-fields{display:grid;grid-template-columns:1fr 1fr;gap:10px}.cxp-quick-title{grid-column:1/-1}.cxp-quick-note{margin-top:12px;padding:10px 11px;border:1px solid #2d1b69;background:#110828;border-radius:10px;color:#a98cc9;font-size:10px;line-height:1.5}.cxp-quick-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:14px}.cxp-inline-error{margin-bottom:10px;padding:9px 10px;border:1px solid rgba(251,113,133,.35);background:rgba(127,29,29,.12);border-radius:9px;color:#fda4af;font-size:11px}.cxp-mobile-status>span,.cxp-lead-status-caption{display:block;margin:0 0 5px;color:#a98cc9;font-size:9px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.cxp-mobile-card-secondary{display:flex;gap:7px}.cxp-mobile-card-actions{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end;gap:7px}.cxp-lead-status-caption{display:none}
@media(max-width:920px){.cxp-quick-create{width:100vw}.cxp-quick-fields{grid-template-columns:1fr}.cxp-quick-title{grid-column:auto}.cxp-quick-actions{display:grid;grid-template-columns:1fr 1fr}.cxp-mobile-card-head{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:start}.cxp-mobile-card-open{width:100%}.cxp-mobile-card-title{display:block;white-space:normal;overflow-wrap:break-word}.cxp-mobile-card-actions{grid-template-columns:1fr}.cxp-mobile-card-secondary{display:grid;grid-template-columns:repeat(3,1fr)}.cxp-mobile-card-secondary .cxp-btn.icon{width:100%;min-width:0}.cxp-lead-head{flex-direction:column;align-items:stretch!important;gap:8px}.cxp-lead-toggle{width:100%;flex:none}.cxp-lead-name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cxp-lead-meta{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cxp-lead-quick{width:100%;padding:0!important;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}.cxp-lead-quick .cxp-btn.icon{width:100%;min-width:0}.cxp-lead-quick .cxp-lead-status-quick{grid-column:1/-1;width:100%}.cxp-lead-status-caption{display:block}.cxp-lead-status-quick select{width:100%!important}.cxp-toolbar>label:not(.grow){flex:1;min-width:0}.cxp-crm-grid{display:flex;overflow-x:auto;scroll-snap-type:x proximity;padding-bottom:6px}.cxp-status-card{flex:0 0 145px;scroll-snap-align:start}.cxp-origin-grid{display:flex;overflow-x:auto;scroll-snap-type:x proximity;padding-bottom:6px}.cxp-origin{flex:0 0 112px;min-height:96px!important;scroll-snap-align:start}}
`;
...
function FilePreview({ raw }: { raw:string }) {
  ...
  useEffect(()=>{if(index>=links.length)setIndex(0)},[links.length,index]);
  if(!links.length)return null;
  const current=links[index]||links[0];
...
function ContentTable({ rows, selected, onSelect, onSelectAll, onOpen, onUpdate, onAdapt, onDuplicate, onRemove }: {
  rows:Row[];selected:Set<string>;onSelect:(id:string)=>void;onSelectAll:()=>void;onOpen:(r:Row)=>void;onUpdate:(id:string,key:keyof Row,val:any)=>void;onAdapt:(r:Row)=>void;onDuplicate:(r:Row)=>void;onRemove:(id:string)=>void;
}) {
...
function MobileContentCards({ rows, selected, onSelect, onOpen, onUpdate, onAdapt, onDuplicate, onRemove }: Parameters<typeof ContentTable>[0]) {
  return <div className="cxp-mobile-cards">{rows.map(r=>{const c=STATUS_COLORS[r.status],progress=r.checklist.length?Math.round(r.checklist.filter(i=>i.done).length/r.checklist.length*100):0;return <article key={r.id} className="cxp-mobile-card" style={{boxShadow:`inset 3px 0 ${c.border}`}}><div className="cxp-mobile-card-head"><input type="checkbox" aria-label={`Selecionar ${r.tema||r.postagem}`} checked={selected.has(r.id)} onChange={()=>onSelect(r.id)} style={{marginTop:3}}/><button type="button" className="cxp-mobile-card-open" onClick={()=>onOpen(r)}><span className="cxp-mobile-card-title">{r.formato===EXTRA_TIKTOK_FORMAT?"🎵 Vídeo extra TikTok":r.tema||r.postagem}</span><span className="cxp-mobile-card-meta"><span>{r.data||"Sem data"}</span><span>{REDE_ICONS[r.rede]||"◆"} {r.rede||"—"}</span><span>{r.formato||"—"}</span></span><UrgencyBadge row={r}/></button></div>{r.checklist.length>0&&<div style={{marginTop:11}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#9a90a8",marginBottom:4}}><span>Objetivos</span><span>{progress}%</span></div><div className="cxp-progress"><div style={{width:`${progress}%`}}/></div></div>}<div className="cxp-mobile-card-actions"><label className="cxp-mobile-status"><span>Status</span><select className="cxp-input" value={r.status} onChange={e=>onUpdate(r.id,"status",e.target.value)} style={{background:c.bg,color:c.text,borderColor:`${c.border}88`}}>{STATUS_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label><div className="cxp-mobile-card-secondary"><button type="button" className="cxp-btn icon" onClick={()=>onDuplicate(r)} aria-label={`Duplicar ${r.tema||r.postagem}`} title="Duplicar">⧉</button><button type="button" className="cxp-btn icon" onClick={()=>onAdapt(r)} aria-label={`Adaptar ${r.tema||r.postagem}`}>↗</button><button type="button" className="cxp-btn icon danger" onClick={()=>onRemove(r.id)} aria-label={`Excluir ${r.tema||r.postagem}`}>×</button></div></div></article>})}</div>;
}
...
function PostDrawer({ row, onClose, onUpdate, onToggleChecklist, onAdapt, onDuplicate, onRemove }: { row:Row;onClose:()=>void;onUpdate:(id:string,key:keyof Row,val:any)=>void;onToggleChecklist:(id:string,itemId:string)=>void;onAdapt:(r:Row)=>void;onDuplicate:(r:Row)=>void;onRemove:(id:string)=>void }) {
...
function QuickPostModal({ initialDate = "", onClose, onCreate }: { initialDate?:string;onClose:()=>void;onCreate:(partial:Partial<Row>,dateISO:string)=>Promise<boolean> }) {
  const ref=useDialogFocus(true,onClose);
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();const title=tema.trim();if(title.length<2){setError("Dê um nome para a postagem.");return}if(saving)return;setSaving(true);setError("");const ok=await onCreate({postagem:title,tema:title,rede,formato,responsavel,status:"Ideia",checklist:makeChecklist(formato)},dateISO);setSaving(false);if(ok)onClose()};
  return <><div className="cxp-overlay" onClick={saving?undefined:onClose} aria-hidden="true"/><section ref={ref} className="cxp-modal cxp-quick-create" role="dialog" aria-modal="true" aria-labelledby="quick-post-title" tabIndex={-1}><form onSubmit={submit}><div className="cxp-modal-head"><div><div className="cxp-section-eyebrow">Criação rápida</div><h2 id="quick-post-title" className="cxp-section-title" style={{margin:0}}>Nova postagem</h2></div><button type="button" className="cxp-btn icon ghost" onClick={onClose} disabled={saving} aria-label="Fechar criação rápida">×</button></div><p className="cxp-quick-copy">Preencha só o essencial. O resto pode ser completado depois, porque cadastrar um post não deveria exigir preencher a declaração do Imposto de Renda.</p>{error&&<div className="cxp-inline-error" role="alert">{error}</div>}<div className="cxp-quick-fields"><label className="cxp-field cxp-quick-title"><span>Tema / título *</span><input autoFocus className="cxp-input" value={tema} onChange={e=>setTema(e.target.value)} placeholder="Ex.: 3 erros que acabam com uma sessão" maxLength={160}/></label><div><div className="cxp-label">Data</div><DatePicker value={dateISO} onChange={setDateISO} label="Data da postagem"/></div><label className="cxp-field"><span>Rede</span><select className="cxp-input" value={rede} onChange={e=>setRede(e.target.value)}><option value="">Escolher depois</option>{REDE_OPTIONS.filter(option=>option!=="Todos").map(option=><option key={option}>{option}</option>)}</select></label><label className="cxp-field"><span>Formato</span><select className="cxp-input" value={formato} onChange={e=>setFormato(e.target.value)}><option value="">Escolher depois</option>{FORMATO_OPTIONS.map(option=><option key={option}>{option}</option>)}</select></label><label className="cxp-field"><span>Responsável</span><input className="cxp-input" value={responsavel} onChange={e=>setResponsavel(e.target.value)} placeholder="Opcional"/></label></div><div className="cxp-quick-note">A postagem entra como <strong>Ideia</strong>. Status, roteiro, CTA, briefing, arquivos e métricas continuam disponíveis na ficha completa.</div><div className="cxp-quick-actions"><button type="button" className="cxp-btn" onClick={onClose} disabled={saving}>Cancelar</button><button type="submit" className="cxp-btn primary" disabled={saving}>{saving?"Criando…":"Criar postagem"}</button></div></form></section></>;
}
...
function LeadsView({ isMobile, openNewNonce = 0 }: { isMobile:boolean;openNewNonce?:number }) {
  ...
  const [expanded,setExpanded]=useState<string|null>(null),[expandedChannel,setExpandedChannel]=useState<string|null>(null);
  const [showNew,setShowNew]=useState(false),[newLead,setNewLead]=useState({nome:"",whatsapp_discord:"",origem:"Manual"});
  ...
  const patch=async(id:string,payload:Partial<Lead>)=>{const before=leads.find(lead=>lead.id===id);setSavingLeadId(id);setError("");setLeads(previous=>previous.map(lead=>lead.id===id?{...lead,...payload}:lead));try{await dbUpdateLead(id,payload);if("status" in payload)copied("Status atualizado") }catch(err){console.error("CRM update failed",err);if(before)setLeads(previous=>previous.map(lead=>lead.id===id?before:lead));setError("A alteração não foi salva. O valor anterior foi restaurado.")}finally{setSavingLeadId(current=>current===id?null:current)}};
  ...
  return <div>
    <SectionTitle eyebrow="Pipeline de comunidade" right={<button type="button" className="cxp-btn primary" onClick={()=>setShowNew(true)}>＋ Novo lead</button>}>Leads & clientes</SectionTitle>
    {copyNotice&&<div className="cxp-inline-success" role="status">✓ {copyNotice}</div>}
    {error&&<div style={{marginBottom:10}}><StatePanel tone="error" title="CRM precisa de atenção" description={error} actionLabel="Recarregar" onAction={()=>void load()}/></div>}
    <div className="cxp-crm-grid" style={{marginBottom:10}}>{LEAD_STATUS_OPTIONS.map(status=>{const color=LEAD_STATUS_COLORS[status],count=leads.filter(lead=>lead.status===status).length;return <button type="button" key={status} onClick={()=>setFilterStatus(filterStatus===status?"Todos":status)} aria-pressed={filterStatus===status} className="cxp-card interactive cxp-status-card" style={{padding:10,color:color.text,borderColor:filterStatus===status?color.border:undefined,textAlign:"left",background:color.bg}}><div style={{font:"800 22px 'Cinzel',serif"}}>{count}</div><div style={{fontSize:10,fontWeight:800,marginTop:3}}>{status}</div></button>})}</div>
    <div className="cxp-origin-grid" style={{marginBottom:10}}>{LEAD_CHANNELS.map(channel=>{const items=channelLeads(channel.chaves),isOpen=expandedChannel===channel.label;return <button type="button" key={channel.label} onClick={()=>setExpandedChannel(isOpen?null:channel.label)} aria-expanded={isOpen} className="cxp-card interactive cxp-origin" style={{color:"inherit",borderColor:isOpen?"#7c3aed":undefined}}><img src={channel.icon} alt=""/><strong>{items.length}</strong><div className="cxp-origin-name">{channel.label}</div></button>})}</div>
    {expandedChannel&&(()=>{const channel=LEAD_CHANNELS.find(item=>item.label===expandedChannel)!,items=channelLeads(channel.chaves);return <div className="cxp-panel cxp-channel-breakdown" style={{padding:12,marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}><img src={channel.icon} alt="" style={{width:24,height:24,objectFit:"contain"}}/><div className="cxp-title" style={{fontSize:11}}>{channel.label} · {items.length} lead{items.length!==1?"s":""}</div></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{LEAD_STATUS_OPTIONS.map(status=><span key={status} className="cxp-chip">{status}: {items.filter(lead=>lead.status===status).length}</span>)}</div></div>})()}
    <div className="cxp-toolbar"><label className="grow"><span className="sr-only">Buscar leads</span><input className="cxp-input" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar nome, contato, origem, campanha…"/></label><label><span className="sr-only">Filtrar leads por status</span><select className="cxp-input" style={{width:isMobile?"100%":190}} value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}><option>Todos</option>{LEAD_STATUS_OPTIONS.map(status=><option key={status}>{status}</option>)}</select></label><button type="button" className="cxp-btn icon" onClick={()=>void load()} aria-label="Atualizar leads">↻</button></div>
    {loading?<LoadingState label="Carregando leads…"/>:filtered.length?<div style={{display:"grid",gap:9}}>{filtered.map(lead=>{const color=LEAD_STATUS_COLORS[lead.status]||LEAD_STATUS_COLORS["Novo lead"],open=expanded===lead.id,wa=whatsappUrl(lead.whatsapp_discord||"");return <article key={lead.id} className="cxp-panel cxp-lead-card" style={{boxShadow:`inset 4px 0 ${color.border}`}}><div className="cxp-lead-head"><button type="button" className="cxp-lead-toggle" onClick={()=>setExpanded(open?null:lead.id)} aria-expanded={open}><span className="cxp-lead-avatar" style={{background:color.bg,color:color.text,borderColor:color.border}}>{(lead.nome||"?").slice(0,1).toUpperCase()}</span><span style={{flex:1,minWidth:0,textAlign:"left"}}><span className="cxp-lead-name">{lead.nome||"—"}</span><span className="cxp-lead-meta">{lead.whatsapp_discord||"sem contato"} · {lead.origem||lead.utm_source||"origem não informada"}</span></span><span aria-hidden="true" className="cxp-muted">{open?"⌃":"⌄"}</span></button><div className="cxp-lead-quick"><CopyButton value={lead.nome||""} label="Copiar nome" compact onFail={()=>setError("Não foi possível copiar o nome.")}/><CopyButton value={lead.whatsapp_discord||""} label="Copiar contato" compact onFail={()=>setError("Não foi possível copiar o contato.")}/><button type="button" className="cxp-btn icon" onClick={()=>void copyLead(lead)} aria-label={`Copiar nome e contato de ${lead.nome||"lead"}`} title="Copiar nome + contato">☷</button>{wa&&<a className="cxp-btn icon green" href={wa} target="_blank" rel="noreferrer" aria-label={`Abrir WhatsApp de ${lead.nome||"lead"}`} title="Abrir WhatsApp">💬</a>}<label className="cxp-lead-status-quick"><span className={isMobile?"cxp-lead-status-caption":"sr-only"}>Status {savingLeadId===lead.id?"· salvando…":""}</span><select value={lead.status} disabled={savingLeadId===lead.id} onChange={e=>void patch(lead.id,{status:e.target.value})} style={{...inputStyle,width:isMobile?"100%":165,background:color.bg,color:color.text,borderColor:`${color.border}88`,fontSize:isMobile?14:10}}>{LEAD_STATUS_OPTIONS.map(status=><option key={status}>{status}</option>)}</select></label></div></div>{open&&<div className="cxp-lead-body"><div className="cxp-lead-contact-strip"><div><div className="cxp-label">Contato rápido</div><strong>{lead.nome||"—"}</strong><span>{lead.whatsapp_discord||"sem contato"}</span></div><div className="cxp-lead-contact-actions"><CopyButton value={lead.nome||""} label="Copiar nome"/><CopyButton value={lead.whatsapp_discord||""} label="Copiar contato"/>{wa&&<a className="cxp-btn green" href={wa} target="_blank" rel="noreferrer">💬 WhatsApp</a>}</div></div><div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,1fr)",gap:9}}><label className="cxp-field"><span>Próxima ação</span><input className="cxp-input" defaultValue={lead.proxima_acao||""} onBlur={e=>void patch(lead.id,{proxima_acao:e.target.value})}/></label><div><div className="cxp-label">Follow-up</div><DatePicker value={lead.follow_up_date||""} onChange={iso=>void patch(lead.id,{follow_up_date:iso})} label="Data de follow-up"/></div><label className="cxp-field"><span>Responsável</span><input className="cxp-input" defaultValue={lead.responsavel||""} onBlur={e=>void patch(lead.id,{responsavel:e.target.value})}/></label><div><div className="cxp-label">Último contato</div><DatePicker value={lead.ultimo_contato||""} onChange={iso=>void patch(lead.id,{ultimo_contato:iso})} label="Data do último contato"/></div><label className="cxp-field"><span>Anotação rápida</span><input className="cxp-input" defaultValue={lead.anotacao_rapida||""} onBlur={e=>void patch(lead.id,{anotacao_rapida:e.target.value})}/></label><label className="cxp-field"><span>Origem</span><input className="cxp-input" defaultValue={lead.origem||""} onBlur={e=>void patch(lead.id,{origem:e.target.value})}/></label><label className="cxp-field"><span>UTM Source</span><input className="cxp-input" defaultValue={lead.utm_source||""} onBlur={e=>void patch(lead.id,{utm_source:e.target.value})}/></label><label className="cxp-field"><span>UTM Campaign</span><input className="cxp-input" defaultValue={lead.utm_campaign||""} onBlur={e=>void patch(lead.id,{utm_campaign:e.target.value})}/></label><label className="cxp-field"><span>Influencer</span><input className="cxp-input" defaultValue={lead.influencer_codigo||""} onBlur={e=>void patch(lead.id,{influencer_codigo:e.target.value})}/></label></div><div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:8,marginTop:11}}>{[["Idade",lead.idade],["Tempo RPG",lead.tempo_rpg],["Sistemas",lead.sistemas_desejados],["Melhor dia",lead.melhor_dia]].map(([label,value])=><div className="cxp-card" style={{padding:9}} key={label}><div className="cxp-label">{label}</div><div style={{fontSize:11,overflowWrap:"anywhere"}}>{value||"—"}</div></div>)}</div></div>}</article>})}</div>:<StatePanel title="Nenhum lead neste filtro" description={leads.length?"Ajuste a busca ou o status para ver outros contatos.":"Novos inscritos e leads manuais aparecerão aqui."} actionLabel={leads.length?"Limpar filtro":"Criar lead"} onAction={()=>leads.length?setFilterStatus("Todos"):setShowNew(true)}/>} 
    {showNew&&<><div className="cxp-overlay" onClick={()=>setShowNew(false)} aria-hidden="true"/><section ref={modalRef} className="cxp-modal" role="dialog" aria-modal="true" aria-labelledby="new-lead-title" tabIndex={-1}><div className="cxp-modal-head"><div><div className="cxp-section-eyebrow">Entrada manual</div><h2 id="new-lead-title" className="cxp-section-title" style={{margin:0}}>Novo lead</h2></div><button type="button" className="cxp-btn icon ghost" onClick={()=>setShowNew(false)} aria-label="Fechar novo lead">×</button></div><div style={{display:"grid",gap:4}}><label className="cxp-field"><span>Nome *</span><input autoFocus className="cxp-input" value={newLead.nome} maxLength={100} onChange={e=>setNewLead(previous=>({...previous,nome:e.target.value}))}/></label><label className="cxp-field"><span>WhatsApp / Discord</span><input className="cxp-input" value={newLead.whatsapp_discord} maxLength={80} onChange={e=>setNewLead(previous=>({...previous,whatsapp_discord:e.target.value}))}/></label><label className="cxp-field"><span>Origem</span><input className="cxp-input" value={newLead.origem} maxLength={100} onChange={e=>setNewLead(previous=>({...previous,origem:e.target.value}))}/></label></div><div style={{display:"flex",justifyContent:"flex-end",gap:7,marginTop:14}}><button type="button" className="cxp-btn" onClick={()=>setShowNew(false)}>Cancelar</button><button type="button" className="cxp-btn primary" disabled={creating} onClick={()=>void create()}>{creating?"Criando…":"Criar lead"}</button></div></section></>}
  </div>;
}
...
function Dashboard({ onVoltar }: { onVoltar:()=>void }) {
  ...
  useEffect(()=>{if(appTab!=="conteudo"&&appTab!=="produtividade")return;void loadRows(mes);setSelectedIds(new Set())},[appTab,mes,loadRows]);
  ...
  const duplicateOne=async(source:Row)=>{
    const iso=dateInputValue(source);
    const row=makeRow(rowsRef.current.length+1,source.mes,iso,{...source,postagem:`${source.postagem} (cópia)`,status:"Ideia",published_at:null,views:0,likes:0,shares:0,saves:0,followers_gained:0,historico:[makeHistory("duplicação",source.id,"nova cópia")]});
  ...
  const openExternalPost=(r:Row)=>{if(r.mes!==mes)setMes(r.mes);if(!rowsRef.current.some(row=>row.id===r.id))setRowsSafe(previous=>[...previous,r]);goTab("conteudo");setSelectedPostId(r.id)};
